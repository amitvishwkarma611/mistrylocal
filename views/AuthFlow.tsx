...
const AuthFlow: React.FC<AuthFlowProps> = ({ onLogin, language, setLanguage, t }) => {
  ...
  useEffect(() => {
    setError(null);
  }, [phone, email, password, otp, selectedRole]);
  ...
  const handleBack = () => {
    if (otpStep) {
      setOtpStep(false);
      setOtp('');
      resetRecaptcha(); // Reset recaptcha when going back
    } else {
      setStep('role');
      setAuthMethod('phone'); // Reset auth method when going back to role selection
    }
  };
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!userDoc.exists()) {
        ...
        name = selectedRole === AppRole.CARPENTER ? "Carpenter User" : "Customer User";
      ...
      onLogin(finalRole!, user.phoneNumber!, name, user.uid);
    } catch (err: any) {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      if (isSigningUp) {
        ...
        const userDocRef = doc(db, "users", user.uid);
        const name = selectedRole === AppRole.CARPENTER ? "Carpenter User" : "Customer User";
        ...
        onLogin(finalRole!, user.email!, name, user.uid);
      }
    } catch (err: any) {
  ...
  return (
    <div className="p-8 min-h-screen animate-in slide-in-from-right-10 duration-500 flex flex-col">
      ...
      <button onClick={handleBack} className="p-2 mb-8 bg-gray-50 rounded-full text-gray-400 hover:text-amber-900 w-fit">
        <ChevronLeft size={24} />
      </button>

      <div className="mb-10">
        <h2 className="text-3xl font-black text-amber-900 mb-2">
          {selectedRole === AppRole.CUSTOMER ? t('login_title_customer') : t('login_title_carpenter')}
        </h2>
        <p className="text-gray-500 font-bold italic">
          {selectedRole === AppRole.CUSTOMER ? t('login_subtitle_customer') : t('login_subtitle_carpenter')}
        </p>
      ...
      <div className="mt-12 text-center">
        <p className="text-sm font-bold text-gray-400 mb-2">{t('signup_prompt')}</p>
        <button className="text-orange-600 font-black underline underline-offset-4">{t('register_now')}</button>
      </div>
