...
const CarpenterAuth: React.FC<CarpenterAuthProps> = ({ onLogin, language, setLanguage, t, selectedProfession }) => {
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let name = "Carpenter User";

      if (!userDoc.exists()) {
        // Create new carpenter user document
      ...
      onLogin(AppRole.CARPENTER, user.phoneNumber!, name, user.uid);
    } catch (err: any) {
    ...
    <div className="p-8 min-h-screen animate-in slide-in-from-right-10 duration-500 flex flex-col">
      ...
      <div className="mb-10">
        <h2 className="text-3xl font-black text-amber-900 mb-2">
          {t('login_title_carpenter')}
        </h2>
        <p className="text-gray-500 font-bold italic">
          {t('login_subtitle_carpenter')}
        </p>
      ...
      {authMethod === 'phone' ? (
        ...
        <form onSubmit={otpStep ? handleVerifyOTP : handleSendOTP} className="space-y-6">
          ...
          <button 
            disabled={loading}
            className="w-full py-5 bg-amber-900 text-white rounded-[2rem] font-black text-lg shadow-xl shadow-amber-100 hover:shadow-amber-200 transition-all active:scale-95 flex items-center justify-center gap-3 disabled:bg-gray-400"
          >
